...
absent species in the mock sample and three non-absent species. The first column
identifies the sample, and remaining columns are
a1, a2, a3 and a4 for the four absent species, and 1, 2 and 3 for the non-absent species.
...
- the number of absent species left in non-mock samples (**check**)
```{r, echo=TRUE}
small_sens_alpha$num.mock.sp.left
```

- the number of non-mock (check**?) samples remaining with non-absent species
```{r, echo=TRUE}
...
- the number of non-mock samples remaining with non-absent species
```{r, echo=TRUE}
